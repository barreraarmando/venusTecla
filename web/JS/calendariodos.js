...
function escribirdias(duracion) {
    ...
    for (i = 1; i < 7; i++) { //localizar fila
        ...
        for (j = 0; j < 7; j++) {
            ...
            if (midia >= ciclo.getDate() - 1 && midia <= ciclo.getDate() + 2) {
                celda.style.backgroundColor = "#FB8385";
            }
            if (midia >= ciclo.getDate() + 1 && midia <= ciclo.getDate() + duracion) {
                celda.style.backgroundColor = "#BC0306";
            }
            if (midia >= ciclo.getDate() + 1 + duracion && midia <= ciclo.getDate() + duracion + 2) {
                celda.style.backgroundColor = "#CF13E8";
            }
            if (midia >= ciclo.getDate() + duracion + 4 && midia <= ciclo.getDate() + duracion + 8) {
                celda.style.backgroundColor = "#7EC4F1";
            }
            //domingos en rojo
            if (j == 6) {
                celda.style.color = "#f11445";
